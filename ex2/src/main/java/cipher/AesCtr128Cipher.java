...
public class AesCtr128Cipher {
    ...
    public void encrypt(Credential credentialToEncrypt, String externalPassword)
        ...
        IvParameterSpec passwordIv = generateInitialisationVector();//génère un IV aléatoire pour le champ 'password'

        String userEncrypted = encryptSingleField(credentialToEncrypt.getUser(), secretKey, userIv); //champ 'user' encrypté
        String passwordEncrypted = encryptSingleField(credentialToEncrypt.getPassword(), secretKey, passwordIv);//champ 'password' encrypté

        credentialToEncrypt.setUser(userEncrypted);//on set le champ 'user' avant la sérialization
        credentialToEncrypt.setPassword(passwordEncrypted);//on set le champ 'password' avant la sérialization
    }
     ...
     * On génère une clef à partir du password. La stratégie est de générer un Hash avec SHA-1 à partir du password claire.
    ...
    private SecretKey generateSecretKeyFromPassword(String password) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        byte[] key = password.getBytes(StandardCharsets.UTF_8); //conversion du password en bytes
        MessageDigest sha = MessageDigest.getInstance(SHA1);
        key = sha.digest(key);//on convertit les bytes en hash avec SHA-1
